...
| `/account/[id]/tx-edit` | `account/[id]/tx-edit.tsx` | Transaction edit screen. Reached by tapping any transaction row. Edit date, amount, description, and category. Delete action semantics differ by source: manual transactions (`source_is_manual = 1`) are hard-deleted; imported transactions (`source_is_manual = 0`) are soft-dropped via `dropped_at` (excluded from summaries, audit trail preserved). |
...
| `/account/[id]/import-dedup` | `account/[id]/import-dedup.tsx` | Cross-import duplicate review screen. Reached from the import done screen when newly imported rows closely match rows from a prior import (same amount, date ±1 day, different batch). Per-pair actions: **Keep New** (soft-drops the existing row), **Keep Existing** (soft-drops the new row), or **Keep Both** (dismiss, no DB write). Both drop options use `softDropTransaction`. |
...
| `tests/db/transaction-mutations.test.ts` | `updateTransaction` changes fields in-place (id unchanged); `hardDeleteTransaction` removes row entirely; `softDropTransaction` sets `dropped_at` and excludes row from summaries; `findCrossImportDuplicates` finds/excludes pairs correctly across batches, amounts, dates, and dropped state. |
